...
begin
  ...
  if ErrCode < 0 then
  begin
    MessageBox(Handle, 'Create Java VM Error', 'Delphi 11 Call Java Class', MB_OK OR MB_ICONERROR);
    Halt;
    Exit;
  end;
  ...
  begin
    MessageBox(Handle, 'Create Java Env Error', 'Delphi 11 Call Java Class', MB_OK OR MB_ICONERROR);
    Exit;
  end;
end;
...
begin
  ...
  if jcls = nil then
  begin
    MessageBox(Handle, 'cant find java class', 'Delphi 11 Call Java Class', MB_OK OR MB_ICONERROR);
    Exit;
  end;
  ...
  if strResult <> '' then
  begin
    MessageBox(Handle, PChar(Format('JavaClassForDelphiTest.goTest  Result: %s', [strResult])), 'Delphi 11 Call Java Class', MB_OK OR MB_ICONINFORMATION);
  end;
end;
